...
(function () {
  ...
  function makeDraggable(panel) {
    ...
    function drag(e) {
      ...
      panel.style.left = "24px";  // Maintain left positioning after drag
  ...
  function showLexiconPanel() {
    ...
    const moodTiles = lexicon.moodTiles || [];
    const patterns = lexicon.promptUpgradePatterns || {};
    const tips = lexicon.quickTips || [];

    // Generate mood tiles HTML
    const tilesHTML = moodTiles.map(tile => `
      <div class="mood-tile" data-mood="${tile.id}">
        <span class="mood-icon">${tile.icon}</span>
        <span class="mood-name">${escapeHtml(tile.name)}</span>
      </div>
    `).join('');
