...
(function(){
  ...
  console.log("[VibeAI] HUD iframe injected");

  // 🛡️ Emergency Pointer-Release (ONE TIME ONLY)
  function ensurePointerRelease() {
    if (emergencyReleaseRun) {
      console.log('[VibeAI] Emergency release already run, skipping');
      return;
    }
    ...
    try {
      ...
      console.log('[VibeAI] 🔓 Emergency pointer-release enforced (ONE TIME ONLY)');
    } catch (err) {
  ...
  function sendBind(){
    if (handshakeComplete) {
      console.log("[VibeAI] Handshake already complete, skipping retry");
      return;
    }
    ...
    retryCount++;
    console.log(`[VibeAI] Binding HUD via postMessage (attempt ${retryCount}/${MAX_RETRIES})`);
  ...
  window.addEventListener("message", e => {
    if (e.data === "VIBEAI_RENDER_READY") {
      ...
      console.log("[VibeAI] ✅ Handshake success — HUD ready");
    ...
    if (e.data === "VIBEAI_ENABLE_IFRAME_CLICKS") {
      ...
      if (iframe) {
        ...
        console.log("[VibeAI] 🔓 Iframe pointer-events ENABLED for modal interaction");
      }
    }
    ...
    if (e.data === "VIBEAI_CONSENT_GRANTED") {
      ...
      if (iframe) {
        iframe.style.setProperty("pointer-events", "none", "important");
        console.log("[VibeAI] 🌊 Consent granted — releasing host page navigation (Phase III-B)");
      }
    }

    // 💫 Handle ESC-key dismissal from HUD
    if (e.data === "VIBEAI_DISMISS_HUD") {
      console.log("[VibeAI] ✨ Dismissing HUD via ESC-key fade-out");
      const hudFrame = document.getElementById(HUD_ID);
      if (hudFrame) {
        hudFrame.remove();
        console.log("[VibeAI] HUD iframe removed cleanly");
      }
    }
  });
