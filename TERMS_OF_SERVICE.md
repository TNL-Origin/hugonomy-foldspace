# VibeAI FoldSpace™ - Terms of Service

**Version:** 1.0.0
**Effective Date:** February 1, 2026
**Status:** Beta Release
**Copyright:** © 2026 Joseph D. Tingling. All Rights Reserved.

---

## 1. Agreement to Terms

By installing and using VibeAI FoldSpace ("the Extension"), you agree to these Terms of Service. If you do not agree, do not install or use the Extension.

## 2. Beta Software Disclaimer

**VibeAI FoldSpace is BETA software.**

- This software is experimental and may contain bugs
- Features may change or be removed without notice
- No guarantee of stability, accuracy, or availability
- Intended for personal testing and feedback only
- Not approved for commercial, mission-critical, or production use

## 3. License Grant

Subject to these Terms, Joseph D. Tingling ("Licensor") grants you a personal, non-exclusive, non-transferable, revocable license to:
- Install and use the Extension on your personal devices
- Use the Extension for personal, non-commercial purposes
- Provide feedback and bug reports

## 4. Restrictions

You may NOT:
- Reverse engineer, decompile, or extract the HugoScore™ algorithm
- Use the Extension for commercial purposes without written permission
- Redistribute, sublicense, or sell copies of the Extension
- Remove or modify copyright notices or attributions
- Use the Extension to violate laws or harm others

## 5. Privacy & Data Processing

**Local Processing Only:**
- All data processing occurs locally in your browser
- No user data is transmitted to external servers
- No telemetry, analytics, or tracking
- Storage uses chrome.storage.local only (no cloud sync)

**What the Extension Accesses:**
- Visible conversation content on supported AI platforms (ChatGPT, Claude, Gemini, Copilot)
- Page metadata (URL, title) to detect platform
- Local storage for UI preferences only

**What We Do NOT Collect:**
- Your conversation content
- Your personal information
- Usage analytics
- Crash reports (unless you submit them manually)

For complete privacy details, see [privacy.html](privacy.html).

## 6. User Responsibilities

**You are solely responsible for:**
- Your use of AI platforms and conversations
- Decisions made based on Extension insights
- Compliance with platform Terms of Service
- Backup of any important data
- Security of your devices and accounts

**The Extension provides emotional intelligence insights ONLY. It does not:**
- Provide professional advice (legal, medical, financial, mental health)
- Make decisions on your behalf
- Guarantee accuracy or completeness of analysis
- Replace human judgment or professional consultation

## 7. Disclaimer of Warranties

THE EXTENSION IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND.

**We disclaim all warranties, including:**
- Merchantability
- Fitness for a particular purpose
- Non-infringement
- Accuracy or reliability
- Uninterrupted or error-free operation

## 8. Limitation of Liability

**TO THE MAXIMUM EXTENT PERMITTED BY LAW:**

Joseph D. Tingling and affiliated entities SHALL NOT BE LIABLE for any:
- Indirect, incidental, special, or consequential damages
- Loss of data, profits, or business opportunities
- Emotional distress or personal injury
- Decisions made based on Extension insights
- Compatibility issues with AI platforms
- Bugs, crashes, or data loss

**MAXIMUM LIABILITY:** In no event shall Joseph D. Tingling's total liability exceed $10 USD.

## 9. Platform Compatibility

**Supported Platforms (Beta):**
- ChatGPT (Chrome, Edge, Firefox)
- Claude.ai (Chrome, Edge, Firefox)
- Google Gemini (Chrome, Edge, Firefox)
- Microsoft Copilot (Chrome, Firefox)

**Known Limitations:**
- Microsoft Edge blocks extensions on copilot.microsoft.com (browser policy, not Extension bug)
- Firefox support is experimental
- Platform updates may break compatibility temporarily

See [PLATFORM_COMPATIBILITY.md](PLATFORM_COMPATIBILITY.md) for details.

## 10. Intellectual Property

**Ownership:**
- Joseph D. Tingling retains all rights to the Extension, HugoScore™ algorithm, and proprietary components
- Hugonomy™ and VibeAI FoldSpace™ are trademarks of Joseph D. Tingling
- Extension code is licensed under MIT License (see [LICENSE.md](LICENSE.md))
- HugoScore™ algorithm is proprietary and distributed in obfuscated form
- **Patent Pending:** U.S. Provisional Patent Application No. 63/856,714

**Your Content:**
- You retain all rights to your conversations and data
- We do not claim ownership of any user-generated content
- We do not store or access your content (local processing only)

## 11. Indemnification

You agree to indemnify and hold harmless Joseph D. Tingling from any claims, damages, or expenses arising from:
- Your use of the Extension
- Your violation of these Terms
- Your violation of third-party rights
- Your violation of AI platform Terms of Service

## 12. Termination

**We may terminate your license if:**
- You violate these Terms
- You reverse engineer proprietary components
- You use the Extension for harmful purposes

**You may terminate by:**
- Uninstalling the Extension
- Clearing all Extension data via browser settings

**Upon termination:**
- Your license immediately ends
- You must uninstall the Extension
- Disclaimers and limitations of liability survive

## 13. Changes to Terms

We may update these Terms at any time. Continued use after changes constitutes acceptance of new Terms. Material changes will be communicated via:
- Extension update notices
- GitHub repository announcements

## 14. Governing Law

These Terms are governed by the laws of the State of Illinois, USA, without regard to conflict of law principles.

## 15. Dispute Resolution

**Informal Resolution:** Contact us first at support@hugonomy.com to resolve disputes informally.

**Arbitration:** Any unresolved disputes shall be settled by binding arbitration under the American Arbitration Association rules.

**Class Action Waiver:** You agree to resolve disputes individually, not as part of a class action.

## 16. Severability

If any provision of these Terms is found unenforceable, the remaining provisions remain in full effect.

## 17. Entire Agreement

These Terms, together with the Privacy Policy and LICENSE.md, constitute the entire agreement between you and Joseph D. Tingling regarding the Extension.

---

## Contact Information

**Developer:** Joseph D. Tingling
**Company:** Hugonomy™
**Email:** support@hugonomy.com
**GitHub:** https://github.com/TNL-Origin/HUGONOMY (private repo)
**Public Repository:** https://github.com/TNL-Origin/vibeai-foldspace-public

---

## Acknowledgment

**BY INSTALLING AND USING VIBEAI FOLDSPACE™, YOU ACKNOWLEDGE THAT:**
1. You have read and understood these Terms
2. You agree to be bound by these Terms
3. The Extension is beta software with no guarantees
4. You use the Extension at your own risk
5. Joseph D. Tingling is not liable for any damages arising from use

---

**Last Updated:** February 1, 2026
**Version:** 1.0.0 (Beta)
