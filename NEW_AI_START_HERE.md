...
**Follow-up, same session, 2026-07-03:** the 12-file patent-pending language flagged above was remediated after Jo explicitly approved cleanup. Surgical per-file patches applied (reusing already-approved phrasing from the local vibeai-foldspace/ 2026-05-14 cleanup pass, which had cleaned these files locally but never pushed them here). Full-repo re-scan afterward confirms zero remaining patent-pending language outside this historical note. — mClaude (Claude Sonnet 5)
