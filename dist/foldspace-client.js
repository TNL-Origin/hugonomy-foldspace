...
if (CLAUDE_DEGRADED_MODE) {
  console.log('[VibeAI] 🧭 Claude Degraded Mode active (read-only HUD, no postMessage binding)');
}
...
if (window.self !== window.top) {
  console.log("[VibeAI] Detected sandboxed iframe — switching to safe-root");
...
function closeConsentModal() {
  ...
  console.log("[VibeAI HUD] 🔄 Starting modal fade-out...");
  modalEl.classList.add("fade-out");
  setTimeout(() => {
    if (modalEl && modalEl.parentNode) {
      modalEl.parentNode.removeChild(modalEl);
      console.log("[VibeAI HUD] ✅ Consent modal fully removed.");
    } else {
...
document.addEventListener("click", (e) => {
  console.log("[VibeAI HUD] 🖱️ GLOBAL CLICK DETECTED:", {
    target: e.target,
...
document.addEventListener("mousedown", (e) => {
  console.log("[VibeAI HUD] 🖱️ MOUSEDOWN DETECTED:", e.target?.id || e.target?.tagName);
}, true);
...
setTimeout(() => {
  ...
  if (yesBtn) {
    console.log("[VibeAI HUD] 🔘 Attaching 'Enable' button listener");
    ...
    const enableHandler = (e) => {
      console.log("[VibeAI HUD] 🖱️ Enable button activated! Event type:", e.type);
      e.preventDefault();
      e.stopPropagation();
      chrome.runtime.sendMessage({ type: "GET_BOOKMARKS" }, res => {
        console.log(`[VibeAI HUD] 📘 Consent granted. Bookmarks: ${res.bookmarks?.length || 0}`);

        // Show the HUD container
        const hudContainer = document.querySelector(".hud-container");
        if (hudContainer) {
          hudContainer.classList.add("visible");
          console.log("[VibeAI HUD] ✅ HUD container now visible");
        }
        ...
        if (toolbar) {
          ...
          console.log("[VibeAI HUD] ✅ Bookmark toolbar now visible");
        }
        ...
        if (inspector) {
          ...
          console.log("[VibeAI HUD] ✅ Thread inspector now visible");
        }
        ...
        if (foldSpaceCanvas) {
          ...
          console.log("[VibeAI HUD] 📖 FoldSpaceCanvas activated");
        }
  ...
  if (noBtn) {
    console.log("[VibeAI HUD] 🔘 Attaching 'Cancel' button listener");
    noBtn.style.setProperty("pointer-events", "auto", "important");
    noBtn.style.setProperty("cursor", "pointer", "important");

    const cancelHandler = (e) => {
      console.log("[VibeAI HUD] 🖱️ Cancel button activated! Event type:", e.type);
      e.preventDefault();
      e.stopPropagation();
      console.log("[VibeAI HUD] ❌ Consent cancelled by user.");
      closeConsentModal();
    };
...
if (!CLAUDE_DEGRADED_MODE) {
  ...
  console.log("[VibeAI] 🧭 Skipping postMessage binding (Claude degraded mode)");
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (reanalyzeBtn) {
    reanalyzeBtn.addEventListener("click", () => {
      console.log("[VibeAI HUD] Re-analyze clicked");
  ...
  if (clearBtn) {
    clearBtn.addEventListener("click", () => {
      console.log("[VibeAI HUD] Clear clicked");
...
document.addEventListener("keydown", e => {
  if (e.key === "Escape") {
    ...
    console.log("[VibeAI HUD] ✨ Dismissing FoldSpace (ESC)");
...
(() => {
  try {
    console.log("[VibeAI HUD] 🛡️ Continuous interactivity guardian active (v2.4.8)");
    setInterval(() => {
      try {
        const body = document.body;
        const currentPE = getComputedStyle(body).pointerEvents;
        if (currentPE !== 'auto') {
          console.log('[VibeAI HUD] 🚨 Guardian detected pointer-events:', currentPE, '→ fixing to auto');
        ...
        buttons.forEach(btn => {
          ...
          if (btnPE !== 'auto') {
            console.log('[VibeAI HUD] 🚨 Button blocked, fixing:', btn.textContent);
...
function loadBookmarks() {
  chrome.runtime.sendMessage({ type: "GET_BOOKMARKS" }, res => {
    ...
    console.log(`[VibeAI HUD] 🗂 Loaded ${res.bookmarks?.length || 0} bookmarks`);
  });
}
...
addBtn.onclick = () => {
  chrome.storage.local.get(["lastThreads"], data => {
    ...
    chrome.runtime.sendMessage({ type: "ADD_BOOKMARK", payload: first }, () => {
      console.log("[VibeAI HUD] 📘 Bookmark added:", first.id);
...
if (toggleCanvasBtn && foldspaceCanvas) {
  toggleCanvasBtn.addEventListener("click", () => {
    ...
    console.log(`[VibeAI] Canvas ${canvasVisible ? 'shown' : 'hidden'}`);
  });
}
...
function renderThreads(list) {
  ...
  console.log(`[VibeAI HUD] 🧠 Rendered ${list.length} threads in inspector`);
...
chrome.storage.onChanged.addListener((changes) => {
  if (changes.lastThreads) {
    console.log("[VibeAI HUD] 🔄 Threads updated, syncing inspector...");
...
threadFeed.addEventListener("click", (e) => {
  ...
  if (card) {
    ...
    console.log("[VibeAI HUD] ✨ Thread card clicked:", threadId, `(${tone})`);
...
function decorateThreads(list) {
  ...
  console.log(`[VibeAI HUD] 💫 Resonance overlay applied to ${analysisCount} threads`);
  updateHugoOrb();
...
function updateHugoOrb() {
  ...
  if (!hriValues.length) {
    ...
    console.log("[VibeAI HUD] ☀️ Resonance orb set to default (no threads)");
    return;
  }
  ...
  console.log(`[VibeAI HUD] ☀️ Average Resonance: ${resonanceLabel} → ${hue} (${hriValues.length} threads)`);
}
...
function applyFocusGlow() {
  ...
  console.log('[VibeAI HUD] 🌟 Focus glow applied to HUD elements');
}
...
setTimeout(() => {
  if (typeof FoldSpaceCanvas !== 'undefined') {
    ...
    console.log('[VibeAI HUD] 📖 FoldSpaceCanvas initialized');
  } else {
...
window.addEventListener("message", (event) => {
  if (event.data.type === "HIGHLIGHT_COMPLETE") {
    ...
    console.log(`[VibeAI HUD] 🌊 Trail spawned: ${threadId} (${tone})`);
  }
});
...
function setHUDInteractive(hudEl, active = true) {
  ...
  if (active) {
    ...
    console.log(`[VibeAI HUD] ${hudEl.id} interactive → true`);

    // Reset auto-deactivate timer
    clearTimeout(autoDeactivateTimer);
    autoDeactivateTimer = setTimeout(() => {
      setHUDInteractive(hudEl, false);
      console.log(`[VibeAI HUD] Auto-deactivated ${hudEl.id} after 10 min idle`);
    }, 600000); // 10 minutes
  } else {
    hudEl.classList.remove('hud-active');
    console.log(`[VibeAI HUD] ${hudEl.id} interactive → false`);
  }
}
...
function releaseAllHUDFocus() {
  ...
  console.log('[VibeAI HUD] ✨ All HUD focus released (safe state)');
}
...
setTimeout(() => {
  ...
  if (inspector) {
    ...
    console.log('[VibeAI HUD] 🎯 Inspector focus control enabled');
  }
  ...
  if (toolbar) {
    ...
    console.log('[VibeAI HUD] 🎯 Toolbar focus control enabled');
  }
}, 1000);
...
function segmentConversation(threads, options = {}) {
  ...
  console.log(`[VibeAI] 📖 ${chapters.length} chapters mapped`);
  return chapters;
}
...
function updateChapters(threads) {
  ...
  chrome.storage.local.set({ chapters }, () => {
    console.log(`[VibeAI] 📖 Stored ${chapters.length} chapters`);
    ...
    console.log(`[VibeAI] 📖 ${chapters.length} chapters mapped | ${resonantPeaks} resonant peak${resonantPeaks !== 1 ? 's' : ''} | variance Δ ${avgVariance.toFixed(3)}`);
  });
...
setTimeout(() => {
  ...
  if (canvas) {
    ...
    console.log('[VibeAI HUD] 📖 Canvas click handler registered');
  }
}, 1000);
...
function ensurePointerEventsOverride() {
  try {
    ...
    if (document.getElementById('vibeai-pointer-events-override')) {
      console.log('[VibeAI HUD] ⚡ Pointer-events override already exists');
      return;
    }
    ...
    console.log('[VibeAI HUD] ✅ Pointer-events safety override applied (post-render)');
  } catch (err) {
