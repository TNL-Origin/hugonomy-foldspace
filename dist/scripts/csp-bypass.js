...
(function injectSafeRoot() {
  try {
    ...
    if (!safeDoc.querySelector("#vibeai-safe-root")) {
      ...
      console.log(`[VibeAI] ${platform} safe-root injected`);
    }
  } catch (err) {
  ...
  try {
    ...
    const waitForRoot = setInterval(() => {
      ...
      if (platformContainer) {
        ...
        console.log(`[VibeAI] 🧭 Anchoring inside ${isClaude ? 'Claude' : isCopilot ? 'Copilot' : 'M365'} visible container:`, platformContainer.tagName);
